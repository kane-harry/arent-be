...
export default class IpfsScheduler implements IScheduler {
    ...
    private async handleNftIpfs() {
        const task = cron.schedule('*/2 * * * *', async () => {
            ...
            const nfts = await NftModel.find({ 'image.ipfs_cid': null, removed: false }, { _id: 0 }).sort({ created: -1 }).limit(10)
    ...
    private async handleCollectionIpfs() {
        const task = cron.schedule('*/2 * * * *', async () => {
            ...
            const collections = await CollectionModel.find({ 'logo.ipfs_cid': null, removed: false }, { _id: 0 }).sort({ created: -1 }).limit(10)
